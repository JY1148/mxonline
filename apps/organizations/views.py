...
class TeacherDetailView(View):
    def get(self, request, teacher_id, *args, **kwargs):
        ...
        if request.user.is_authenticated:
            if UserFav.objects.filter(user=request.user, fav_type=3, fav_id=teacher.id):
                teacher_fav = True
            if UserFav.objects.filter(user=request.user, fav_type=2, fav_id=teacher.org.id):
                org_fav = True
...
class OrgView(View):
    def get(self, request, *args, **kwargs):
        all_orgs = CourseOrg.objects.all()#从数据库中提取数据,将all_orgs定义为Courseorg之下的所有项目
        ...
        org_nums = all_orgs.count()  # 同样可以计算机构数量 org_nums = len(all_orgs)
        #对课程机构进行分页
        try:
            page = request.GET.get('page', 1)
        except PageNotAnInteger:
        ...
        orgs = p.page(page)
        #↓为org_list.html传递变量（”html中的名称“：上文引入的变量）↓
